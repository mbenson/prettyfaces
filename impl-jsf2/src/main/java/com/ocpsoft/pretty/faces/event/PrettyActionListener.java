...
public class PrettyActionListener implements ActionListener {
    ...
    public static class Install implements SystemEventListener {
        ...
        public void processEvent(SystemEvent event) throws AbortProcessingException {
            ...
            ActionListener delegate = app.getActionListener();
            if (delegate instanceof PrettyActionListener) {
                //TODO WTF
            }
    ...
    public void processAction(ActionEvent event) throws AbortProcessingException {
        ...
        try {
            expression = getExpression((ActionSource) event.getComponent());
            final UrlMapping mapping = PrettyContext.getCurrentInstance().getConfig().getMappingById(expression);
            if (mapping != null) {
                new ParameterHandler(FacesContext.getCurrentInstance(), event.getComponent(), mapping).handle();
            }
        } catch (Exception e) {
            LOG.error("Problem applying action UIParameters to mapping " + expression, e);
        }
        delegate.processAction(event);
    }

    @SuppressWarnings("deprecation")
    private static String getExpression(ActionSource action) {
        if (action instanceof ActionSource2) {
            MethodExpression e = ((ActionSource2) action).getActionExpression();
            return e == null ? null : e.getExpressionString();
        }
        MethodBinding b = action.getAction();
        return b == null ? null : b.getExpressionString();
    }
    ...
    private static class ParameterHandler {
        ...
        ParameterHandler(FacesContext context, UIComponent component, UrlMapping mapping) {
            super();
        ...
        private void handleNamedParams() {
            comp: for (UIParameter param : URL_BUILDER.extractParameters(component)) {
        ...
        private void handlePathParams() {
            ...
            for (UIParameter param : URL_BUILDER.extractParameters(component)) {
